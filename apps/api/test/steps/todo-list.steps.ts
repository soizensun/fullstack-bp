...
class TodoWorld extends World {
  ...
  /**
   * BE_09 R8 — every error response nests under `error`. `lastBody` stays a loose
   * `Record` because a success body has no fixed shape here; this reads the one field
   * these steps assert on.
   */
  get lastErrorCode(): unknown {
    const error = this.lastBody?.error as { code?: unknown } | undefined;
    return error?.code;
  }
...
Then(
  ...
  function (this: TodoWorld) {
    ...
    assert.equal(this.lastErrorCode, 'TODO_ITEM_TITLE_DUPLICATE');
  },
);

Then('I am told the list is archived and cannot be changed', function (this: TodoWorld) {
  assert.equal(this.lastStatus, 409);
  assert.equal(this.lastErrorCode, 'TODO_LIST_ARCHIVED_NOT_MODIFIABLE');
});
